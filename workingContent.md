...
Multiple people creating data in the field poses a hard technical problem: our “web 2.0” environment presumes constant connectivity, data “authority” held by centralised servers, and considers mobile devices tools for presentation rather than tools for creation. A particular design challenge is the remoteness of the sampling locations, hundreds of kilometres away from network access. The alternative, however, is handwritten collection with a lengthy, error prone, and expensive digitisation process. 

FAIMS Mobile represents a generalised solution for digital field data collection[^footnote1]. This software, originally created by a community of archaeologists, had to accommodate diverse recording practices. This community agreed that agreed that “recording born-data in the field’’ was desireable, but did not share standard vocabularies, workflows, attributes, or methodologies.  

In response, the FAIMS Project created an open-source, generalised, and deeply customisable mobile platform (Android application supported by Ubuntu server) that allows arbitrary data collection in the field[^footnote2]. To do so, FAIMS relied heavily on existing OSS projects including: spatialite and gdal (for GIS support), sqlite (for a lightweight key-attribute-value datastore), Javarosa and Beanshell (for UI and scripting), and the entire linux stack plus ruby for a server[^footnote3]. 

While single-purpose mobile apps (i.e. those commissioned by structural geologists to take strikes and dips) may excel at their specific tasks, many other geoscientists (e.g., palaeoecologists, palaeontologists, or anyone collecting samples), likely cannot afford to commission and maintain dedicated recording tools. FAIMS Mobile meets these needs, both for rapid prototyping and deployment. FAIMS demonstrates the utility of generalised, OSS software development to provide geoscientists with flexible digital field data collection for a variety of tasks. Customisation is accomplished through "definition documents" - separate from the core" software - that can be shared and reused. Finally, the data is open and exports are scriptable, so interoperability with other data formats and services is both encouraged and possible. 

This poster describes the internal architecture of the FAIMS app, shows how it was used by CSIRO in the field, and maps its OSS heritage. FAIMS Mobile is available from Google Play, with source code (core software, definition documents, and customised exporters can be found on GitHub. You can make your own data-collection modules at no cost via the user to developer documentation at https://www.fedarch.org/support/2, or contact the FAIMS Project for support.
